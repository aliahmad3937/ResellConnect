        ...
        fun showCometChatErrorDialog (context: Context?, errorMessage : String?) {
            try {
                ...
                val dialogView = LayoutInflater.from(context)
                    .inflate(R.layout.cometchat_error_message_view, null, false)
                builder?.setView(dialogView)
                dialogView.findViewById<TextView>(R.id.tv_error_message).text = errorMessage
                dialogView.findViewById<LinearLayout>(R.id.ll_background).background =
                    context?.getDrawable(R.color.red_600)
                dialogView.findViewById<ImageView>(R.id.iv_error_icon)
                    .setImageResource(R.drawable.error_icon)
                val alertDialog = builder?.create()
                alertDialog?.window?.setGravity(Gravity.TOP)
                alertDialog?.window?.attributes?.windowAnimations = R.style.DialogAnimation
                dialogView.findViewById<ImageView>(R.id.iv_error_close)
